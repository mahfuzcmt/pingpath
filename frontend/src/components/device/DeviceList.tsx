...
export function DeviceList({ devices, locations, selectedImei, onSelect }: DeviceListProps) {
  ...
  const sorted = useMemo(() => {
    ...
    return devices
      ...
      .sort((a, b) => {
        // ONLINE first, then by speed (moving first), then by last-seen
  ...
  return (
    <aside className="flex h-full w-80 shrink-0 flex-col border-r border-ink-400/15 bg-ink-900/60">
      {/* Status filter tabs */}
      <div className="flex border-b border-ink-400/15 text-xs">
